...
LOGGER = logging.getLogger("hermes_orchestrator.observability")
...
def build_forward_headers(request: Request) -> dict[str, str]:
    ...
    org_id = _normalize_optional_string(request.headers.get("x-org-id"))
    if org_id is not None:
        headers["x-org-id"] = org_id
    viewer_role = _normalize_optional_string(request.headers.get("x-viewer-role"))
    if viewer_role is not None:
        headers["x-viewer-role"] = viewer_role
    auth_user = _normalize_optional_string(request.headers.get("x-auth-user"))
